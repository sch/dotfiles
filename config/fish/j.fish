...
#   * j -r foo  # goes to highest ranked dir matching foo
#   * j -t foo  # goes to most recently accessed dir matching foo
#   * j -l foo  # list all dirs matching foo (by frecency)
...
function __j_init -d 'Set up automatic population of the directory list for j'
	functions fish_prompt | grep -q 'j --add'
	if [ $status -gt 0 ]
